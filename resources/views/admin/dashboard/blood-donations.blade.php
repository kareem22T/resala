...
@section('title', '     طلبات التبرع بالدم ')
@section('blood_donations_active', 'active')

@section('content')
<h3 class="mb-5 d-flex w-100 justify-content-between">
    طلبات التبرع بالدم
    <a href="/admin/export-blood-donations" class=" btn btn-success">اخراج اكسيل</a>
</h3>
            ...
            <table class="table text-nowrap mb-0 align-middle">
                <thead class="text-dark fs-4">
                    <tr>
                        ...
                        <th class="border-bottom-0">
                            <h6 class="fw-semibold mb-0 d-inline-flex align-items-center">العمر</h6>
                        </th>
                        <th class="border-bottom-0">
                            <h6 class="fw-semibold mb-0 d-inline-flex align-items-center">العنوان</h6>
                        </th>
                        <th class="border-bottom-0">
                            <h6 class="fw-semibold mb-0 d-inline-flex align-items-center">فصلية الدم</h6>
                        </th>
                        <th class="border-bottom-0">
                ...
                <tbody>
                    <tr v-for="(volunteer, index) in unseenedvolunteers" :key="index">
                        ...
                        <td class="border-bottom-0"><h6 class="fw-semibold mb-0">@{{volunteer.age}}</h6></td>
                        <td class="border-bottom-0"><h6 class="fw-semibold mb-0">@{{volunteer.address}}</h6></td>
                        <td class="border-bottom-0"><h6 class="fw-semibold mb-0">@{{volunteer.blood_type}}</h6></td>
...
createApp({
    ...
    methods: {
        async deletevolunteer(volunteer_id) {
            ...
            try {
                const response = await axios.post(`{{ route("blood_donations.delete") }}`, {
        ...
        async getUnseenedvolunteers() {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.get(`{{ route("blood_donations.getunseen") }}?page=${this.unseenpage}`, {
                },
                );
                if (response.data.status === true) {
                    $('.loader').fadeOut()
        ...
        async getvolunteers() {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`{{ route("blood_donations.get") }}?page=${this.page}`, {
                },
                );
                if (response.data.status === true) {
                    $('.loader').fadeOut()
        ...
        async getSearch(search_words) {
            try {
                const response = await axios.post(`{{ route("blood_donations.search") }}?page=${this.page}`, {
        ...
        async seeUnseened() {
            try {
                const response = await axios.post(`{{ route("blood_donations.see") }}`
                );
                if (response.data.status === true) {
                } else {
                    $('.loader').fadeOut()
