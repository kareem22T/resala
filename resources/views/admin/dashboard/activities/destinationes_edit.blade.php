    ...
    <div class="card w-100">
        <div class="card-body p-4">
            <form @submit.prevent>
                ...
                <div class="d-flex gap-3">
                    <div class="w-50">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div contenteditable="true" id="article-content" class="form-control" style="min-height: 300px">{!! $destination->description !!}</div>
                            </div>
                        </div>
                    </div>
                    <div class="w-50">
                ...
                <button class="btn btn-primary mt-3" @click="getContentArticle().then(() => { update() })"><i class="ti ti-edit"></i> تحديث</button>
            </form>
...
createApp({
    data() {
        return {
            ...
            description: null,
            preview_img:  "/dashboard/images/uploads/{{ $destination->thumbnail->path }}",
            showImages: false,
    ...
    methods: {
        async update() {
            ...
            try {
                const response = await axios.post(`{{ route("activites.update") }}`, {
                    title: this.title,
                ...
                if (response.data.status === true) {
                    ...
                    setTimeout(() => {
                        $('#errors').fadeOut('slow')
                        window.location.href = '{{ route("activites.prev") }}'
                    }, 3500);
