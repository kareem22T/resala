...
@section('pages_add_active', 'active')
...
<div class="card" id="add_page">
    <div class="card-body" >
        <div>
            <div class="w-100 mb-4 pb-3 gap-2" style="display: grid; grid-template-columns: 1fr 1fr;">
                <div class="w-100">
                    <div>
            ...
            <div class="w-100 mb-4 pb-3 gap-2" style="display: grid; grid-template-columns: 1fr 1fr;">
                <div class="w-100">
                    <div>
                        <label for="url" class="form-label">رابط الصفحة *</label>
            ...
            <div class="w-100 mb-4 pb-3">
                <div class="w-100 p-3">
                    <div>
                        <label for="lang_name" class="form-label">محتوى الصفحة *</label>
                        <div class="card">
                            <div class="card-header">
                                <div class="toolbar d-flex gap-1 justify-content-center" style="flex-wrap: wrap">
                                    ...
                                    <button @click="insertHTML('<h2>Heading</h2>', 'page-content')" class="btn btn-success"><i class="ti ti-h-2"></i></button>
                                    <button @click="insertHTML('<h3>Heading</h3>', 'page-content')" class="btn btn-success"><i class="ti ti-h-3"></i></button>
                                    <button @click="insertHTML('<h4>Heading</h4>', 'page-content')" class="btn btn-success"><i class="ti ti-h-4"></i></button>
                                    <button @click="insertHTML('<h5>Heading</h5>', 'page-content')" class="btn btn-success"><i class="ti ti-h-5"></i></button>
                                    <button @click="insertHTML('<h6>Heading</h6>', 'page-content')" class="btn btn-success"><i class="ti ti-h-6"></i></button>
                                    <button @click="insertHTML('<p>Paragraph</p>', 'page-content')" class="btn btn-success">P</button>
                                    ...
                                    <button class="btn btn-success" @click="setValuesToNull();this.showImages = true; this.current_page_id = 'page-content'"><i class="ti ti-photo-plus"></i></button>
                                    <button class="btn btn-success" @click="this.showCodePopUp = true"><i class="ti ti-code"></i></button>
                                </div>
                            </div>
                            <div class="card-body">
                                <div contenteditable="true" id="page-content" class="form-control" style="min-height: 300px">
                                {!! $page->content !!}
                                </div>
                            </div>
                        </div>
            ...
            <div class="mb-3 w-100">
                <div class="w-25">
                    ...
                    <div class="w-100 mb-3 pb-5">
                        <button type="submit" class="btn btn-primary w-50 form-control" style="height: fit-content" @click="this.getContentpage().then(() => {this.update()})"><i class="ti ti-edit"></i> تحديث</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="pop-up show-slider-pop-up card" v-if="showSliderPopUp" style="padding: 1rem;width: 730px;display: flex;flex-direction: column;justify-content: center;align-items: center;max-width: 90vw;gap: 20px;">
        ...
        <div class="d-flex gap-2">
            ...
            <button class="btn btn-secondary" @click="insertSliderContent('page-content')">insert</button>
        </div>
    </div>
    ...
    <div class="pop-up show-slider-pop-up card" v-if="showAlbumPopUp" style="padding: 1rem;width: 730px;display: flex;flex-direction: column;justify-content: center;align-items: center;max-width: 90vw;gap: 20px;">
        ...
        <div class="d-flex gap-2">
            ...
            <button class="btn btn-secondary" @click="insertAlbumContent('page-content')">insert</button>
        </div>
    </div>
    ...
    <div class="pop-up show-images-pop-up card" v-if="showImages" style="min-width: 90vw;height: 90vh;padding: 20px;display: flex;flex-direction: column;justify-content: space-between;gap: 1rem;">
        ...
        <div class="foot" style="display: flex;width: 100%;justify-content: space-between;gap: 1rem;">
            ...
            <div class="d-flex gap-2">
                ...
                <button class="btn btn-success"  @click="insertImgTopage();this.forSlider = false">Choose</button>
            </div>
        </div>
    </div>
    ...
    <div class="pop-up show-code-pop-up card" v-if="showCodePopUp" style="padding: 1rem;width: 730px;display: flex;flex-direction: column;justify-content: center;align-items: center;max-width: 90vw;gap: 20px;">
        ...
        <div class="d-flex gap-2">
            ...
            <button class="btn btn-secondary" @click="insertHTML(this.code, 'page-content');this.showCodePopUp = false">insert</button>
        </div>
    </div>
</div>
@endsection
...
createApp({
  data() {
    return {
      ...
      search_tags: null,
      preview_img: '{{ $page->thumbnail_path }}',
      search: null,
      page: 1,
      total: 0,
      last_page: 0,
      ...
      forAlbum: false,
      type: '{{ $page->type }}',
      publish_date: new Date('{{ $page->created_at }}').toISOString().split('T')[0],
      url: '{{ $page->url }}'
    }
  },
  methods: {
    ...
    async update() {
      $('.loader').fadeIn().css('display', 'flex')
        try {
            const response = await axios.post(`{{ route('pages.update') }}`, {
                id: this.page_id,
                title: this.title,
            ...
            setTimeout(() => {
                $('#errors').fadeOut('slow')
                window.location.href = '{{ route("pages.prev") }}'
            }, 2000);
            } else {
            $('.loader').fadeOut()
            document.getElementById('errors').innerHTML = ''
    ...
    insertImgTopage () {
        if (this.current_page_id) {
            if (this.choosed_img) {
                this.insertHTML('<img src="' + this.choosed_img + '" />', this.current_page_id)
                this.chooseImage = null
                this.current_page_id = null
                this.showImages = null
            }else {
    ...
    async getContentpage() {
        ...
        if (document.getElementById('page-content') && document.getElementById('page-content').innerHTML != '')
            this.content = document.getElementById('page-content').innerHTML;
    },
  },
  created() {
    this.getImages()
